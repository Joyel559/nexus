...
def _resolve_claude_command(configured_bin: str) -> str | None:
    """Resolve Claude CLI command even when shell PATH is incomplete."""

    if resolved := shutil.which(configured_bin):
        return resolved

    home = Path.home()
    candidates = []
    if "/" in configured_bin:
        candidates.append(Path(configured_bin).expanduser())
    else:
        candidates.extend(
            [
                home / ".local" / "bin" / configured_bin,
                home / ".npm-global" / "bin" / configured_bin,
                home / ".volta" / "bin" / configured_bin,
            ]
        )
...
def launch_claude(argv: Sequence[str] | None = None) -> None:
    """Launch Claude Code with Nexus proxy env variables."""
    ...
    claude_command = _resolve_claude_command(settings.claude_cli_bin)
...
def migrate_credentials() -> None:
    """Import legacy provider credentials from env into encrypted gateway DB."""

    settings = get_settings()
    db_path = settings.gateway_state_db_path
    run_migrations(
        db_path,
        backend=settings.gateway_storage_backend,
        postgres_dsn=settings.gateway_postgres_dsn,
    )
    ...
    try:
        pool = ProviderPoolManager(
            db,
            cipher=CredentialCipher(settings.gateway_encryption_key),
        )
        migrator = CredentialMigrator(pool)
        migrated = migrator.migrate_from_settings(settings)
        print(f"Migrated {len(migrated)} credential(s).")
        for item in migrated:
            print(
                f"- provider={item.provider_id} source={item.source_key} account_id={item.account_id}"
            )
    finally:
        db.close()
