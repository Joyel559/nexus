...
class ProviderOverrides:
    """Optional runtime overrides used by gateway account routing."""
...
def build_provider_config(
    ...
    *,
    ...
    if overrides is not None and overrides.proxy is not None:
        proxy = overrides.proxy
    enable_thinking = settings.enable_model_thinking
    if overrides is not None and overrides.enable_thinking is not None:
...
def create_provider(
    ...
    *,
    ...
    config = build_provider_config(descriptor, settings, overrides=overrides)
...
class ProviderRegistry:
    ...
    def __init__(self, providers: MutableMapping[str, BaseProvider] | None = None):
        ...
        self._overridden_providers: dict[
            tuple[str, str | None, str | None, str | None, bool | None], BaseProvider
        ] = {}
    ...
    def get(
        ...
        *,
        ...
        if overrides is not None:
            ...
            if cache_key not in self._overridden_providers:
                self._overridden_providers[cache_key] = create_provider(
                    provider_id, settings, overrides=overrides
                )
            return self._overridden_providers[cache_key]
    ...
    async def cleanup(self) -> None:
        ...
        overridden_items = [
            (f"{provider_id}:override", provider)
            for (provider_id, *_), provider in self._overridden_providers.items()
        ]
        errors: list[Exception] = []
        try:
            for _pid, provider in [*items, *overridden_items]:
                try:
            ...
            self._overridden_providers.clear()
