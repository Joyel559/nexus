...
def clone_body_without_tools(body: dict[str, Any]) -> dict[str, Any] | None:
    """Clone a request body and strip tool declarations/history for text-only fallback."""
    cloned_body = deepcopy(body)
    changed = False

    if "tools" in cloned_body:
        cloned_body.pop("tools", None)
        changed = True
    if "tool_choice" in cloned_body:
        cloned_body.pop("tool_choice", None)
        changed = True
    ...
    if isinstance(messages, list):
        ...
        for message in messages:
            ...
            # OpenAI chat requires assistant messages to carry content when no tool_calls
            # are present. Drop now-empty assistant tool-only placeholders.
            if role == "assistant":
        ...
        if sanitized_messages != messages:
            cloned_body["messages"] = sanitized_messages
            changed = True
