...
class NvidiaNimProvider(OpenAIChatTransport):
    ...
    def _get_retry_request_body(self, error: Exception, body: dict) -> dict | None:
        ...
        if not isinstance(error, openai.BadRequestError) and status_code != 400:
            if isinstance(status_code, int) and status_code in {404, 410, 422}:
                fallback_steps = (
                    ("tools", clone_body_without_tools),
                    ("reasoning_budget", clone_body_without_reasoning_budget),
                    ("chat_template_kwargs", clone_body_without_chat_template_kwargs),
                    ("chat_template", clone_body_without_chat_template),
                    ("reasoning_content", clone_body_without_reasoning_content),
                )
                for step_name, step in fallback_steps:
                    retry_body = step(body)
                    if retry_body is None:
                        continue
                    logger.warning(
                        "NIM_STREAM: retrying without {} after status={} incompatibility",
                        step_name,
                        status_code,
                    )
                    return retry_body
        ...
        if (
            "function" in error_text and "not found" in error_text
        ) or "unsupported tool" in error_text:
            retry_body = clone_body_without_tools(body)
            if retry_body is None:
                return None
            logger.warning(
                "NIM_STREAM: retrying without tools after upstream tool incompatibility"
            )
            return retry_body
