...
class OpenAIChatTransport(BaseProvider):
    ...
    async def _create_stream(self, body: dict) -> tuple[Any, dict]:
        ...
        attempt_body = body
        seen_retry_bodies: set[str] = set()
        max_attempts = 4
        for _ in range(max_attempts):
            try:
                create_body = self._prepare_create_body(attempt_body)
                stream = await self._global_rate_limiter.execute_with_retry(
                    self._client.chat.completions.create, **create_body, stream=True
                )
                return stream, attempt_body
            except Exception as error:
                retry_body = self._get_retry_request_body(error, attempt_body)
                if retry_body is None:
                    raise
                retry_signature = json.dumps(
                    retry_body, sort_keys=True, default=str
                )
                if retry_signature in seen_retry_bodies:
                    raise
                seen_retry_bodies.add(retry_signature)
                attempt_body = retry_body
        raise RuntimeError("Provider stream retry budget exhausted")
