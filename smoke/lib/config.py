...
def _normalize_provider_model(provider: str, raw_model: str) -> str:
    ...
    if provider == "open_router":
        ...
        if prefix in disallowed_nested_prefixes:
            msg = (
                f"FCC_SMOKE_MODEL_{provider.upper()} must use provider prefix "
                f"{provider!r}, got {model!r}"
            )
            raise ValueError(msg)
        # OpenRouter model ids are commonly provider-qualified (for example
        # ``openai/gpt-oss-120b:free``), so nested vendor prefixes are valid.
        return f"{provider}/{model}"
