...
_GENERIC_TOKEN_RE = re.compile(r"\b(sk|rk|ghp|gho|ghu)_[A-Za-z0-9_\-]{12,}\b")
    ...
    if isinstance(value, Mapping):
        ...
        for key, item in value.items():
            ...
            if key_text.lower() in _SECRET_KEYS:
                ...
                out[key_text] = _sanitize_extra(item)
        return out
    if isinstance(value, list | tuple):
        return [_sanitize_extra(item) for item in value]
    if isinstance(value, str):
        return _redact_sensitive_substrings(value)
    return value
...
def _serialize_with_context(record) -> str:
    ...
    if isinstance(trace_payload, dict):
        for tk, tv in trace_payload.items():
            if tk in out:
                continue
            out[tk] = _sanitize_extra(tv)
        out["trace"] = True
    for key, value in extra.items():
        if key in _CONTEXT_KEYS or key == _TRACE_PAYLOAD_BINDING:
            continue
        if key in out:
            continue
        out[key] = _sanitize_extra(value)
