...
def _env_files() -> tuple[Path, ...]:
    ...
    files: list[Path] = [
        Path.home() / ".config" / "nexus" / ".env",
        # Keep project-local .env highest priority for repo runs.
        Path(".env"),
    ]
...
class Settings(BaseSettings):
    ...
    google_oauth_scopes: str = Field(
        default="openid email profile https://www.googleapis.com/auth/generative-language",
    ...
    gateway_state_db_path: str = Field(
        default=".config/nexus/gateway.db",
        validation_alias="GATEWAY_STATE_DB_PATH",
    )
    gateway_storage_backend: str = Field(
        default="sqlite",
        validation_alias="GATEWAY_STORAGE_BACKEND",
    )
    gateway_postgres_dsn: str | None = Field(
        default=None,
        validation_alias="GATEWAY_POSTGRES_DSN",
    )
    gateway_routing_config: str = Field(
        default="{}",
        validation_alias="GATEWAY_ROUTING_CONFIG",
    )
    gateway_encryption_key: str = Field(
        default="",
        validation_alias="GATEWAY_ENCRYPTION_KEY",
    )
    gateway_queue_max_inflight: int = Field(
        default=128,
        validation_alias="GATEWAY_QUEUE_MAX_INFLIGHT",
    )
    gateway_queue_max_queued: int = Field(
        default=512,
        validation_alias="GATEWAY_QUEUE_MAX_QUEUED",
    )
    gateway_queue_acquire_timeout_ms: int = Field(
        default=30000,
        validation_alias="GATEWAY_QUEUE_ACQUIRE_TIMEOUT_MS",
    )
    gateway_queue_backend: str = Field(
        default="local",
        validation_alias="GATEWAY_QUEUE_BACKEND",
    )
    gateway_redis_url: str | None = Field(
        default=None,
        validation_alias="GATEWAY_REDIS_URL",
    )
    gateway_redis_queue_prefix: str = Field(
        default="fcc-gateway",
        validation_alias="GATEWAY_REDIS_QUEUE_PREFIX",
    )
    gateway_event_bus_backend: str = Field(
        default="local",
        validation_alias="GATEWAY_EVENT_BUS_BACKEND",
    )
    gateway_redis_event_channel: str = Field(
        default="fcc-gateway-events",
        validation_alias="GATEWAY_REDIS_EVENT_CHANNEL",
    )
    gateway_public_base_url: str | None = Field(
        default=None,
        validation_alias="GATEWAY_PUBLIC_BASE_URL",
    )
    gateway_retention_days: int = Field(
        default=14,
        validation_alias="GATEWAY_RETENTION_DAYS",
    )
    agents_repo_paths: str = Field(
        default="",
        validation_alias="AGENTS_REPO_PATHS",
    )
    agents_sync_targets: str = Field(
        default="",
        validation_alias="AGENTS_SYNC_TARGETS",
    )
    agents_custom_root: str = Field(
        default="",
        validation_alias="AGENTS_CUSTOM_ROOT",
    )
    agents_autodetect_on_startup: bool = Field(
        default=True,
        validation_alias="AGENTS_AUTODETECT_ON_STARTUP",
    )
    agents_auto_sync_on_startup: bool = Field(
        default=False,
        validation_alias="AGENTS_AUTO_SYNC_ON_STARTUP",
    )
    ...
    @classmethod
    def parse_oauth_secrets(cls, v: Any) -> str:
        if v is None:
            return ""
        if isinstance(v, str):
            return v
    ...
    @field_validator(
        "gateway_queue_max_inflight",
        "gateway_queue_max_queued",
        "gateway_queue_acquire_timeout_ms",
    )
    @classmethod
    def validate_gateway_queue_limits(cls, v: int) -> int:
        if v <= 0:
            raise ValueError("gateway queue limits must be > 0")
        return v

    @field_validator("gateway_retention_days")
    @classmethod
    def validate_gateway_retention_days(cls, v: int) -> int:
        if v <= 0:
            raise ValueError("gateway retention days must be > 0")
        return v

    @field_validator("gateway_storage_backend")
    @classmethod
    def validate_gateway_storage_backend(cls, v: str) -> str:
        normalized = v.strip().lower()
        if normalized not in {"sqlite", "postgres"}:
            raise ValueError("gateway_storage_backend must be sqlite or postgres")
        return normalized

    @field_validator("gateway_queue_backend")
    @classmethod
    def validate_gateway_queue_backend(cls, v: str) -> str:
        normalized = v.strip().lower()
        if normalized not in {"local", "redis"}:
            raise ValueError("gateway_queue_backend must be local or redis")
        return normalized

    @field_validator("gateway_event_bus_backend")
    @classmethod
    def validate_gateway_event_bus_backend(cls, v: str) -> str:
        normalized = v.strip().lower()
        if normalized not in {"local", "redis"}:
            raise ValueError("gateway_event_bus_backend must be local or redis")
        return normalized
    ...
    @field_validator("gateway_routing_config")
    @classmethod
    def validate_gateway_routing_config(cls, v: str) -> str:
        text = v.strip()
        if not text:
            return "{}"
        try:
            ...
            raise ValueError("gateway_routing_config must be valid JSON") from exc
        if not isinstance(parsed, dict):
            raise ValueError("gateway_routing_config must be a JSON object")
        return text
    ...
    def check_nvidia_nim_api_key(self) -> Settings:
        ...
        if self.gateway_queue_backend == "redis" and not (
            self.gateway_redis_url or ""
        ).strip():
            raise ValueError(
                "GATEWAY_REDIS_URL is required when GATEWAY_QUEUE_BACKEND=redis."
            )
        if self.gateway_event_bus_backend == "redis" and not (
            self.gateway_redis_url or ""
        ).strip():
            raise ValueError(
                "GATEWAY_REDIS_URL is required when GATEWAY_EVENT_BUS_BACKEND=redis."
            )
        return self
