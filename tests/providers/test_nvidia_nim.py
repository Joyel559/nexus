...
def _make_status_error(status_code: int, message: str) -> openai.APIStatusError:
...
@pytest.mark.asyncio
async def test_stream_response_retries_multiple_410_compat_steps(provider_config):
    provider = NvidiaNimProvider(
        provider_config,
        nim_settings=NimSettings(chat_template="custom_template"),
    )
    ...
    first_error = _make_status_error(410, "unsupported tool payload")
    second_error = _make_status_error(410, "unsupported reasoning budget")
    third_error = _make_status_error(410, "unsupported thinking template")

    with patch.object(
        provider._client.chat.completions, "create", new_callable=AsyncMock
    ) as mock_create:
        mock_create.side_effect = [first_error, second_error, third_error, mock_stream()]
        events = [e async for e in provider.stream_response(req)]
    ...
    third_call = mock_create.call_args_list[2].kwargs
    fourth_call = mock_create.call_args_list[3].kwargs

    assert "tools" in first_call
    assert "tools" not in second_call
    assert (
        second_call.get("extra_body", {})
        .get("chat_template_kwargs", {})
        .get("reasoning_budget")
        is not None
    )
    assert "chat_template_kwargs" in third_call.get("extra_body", {})
    assert "chat_template_kwargs" not in fourth_call.get("extra_body", {})
...
@pytest.mark.asyncio
async def test_stream_response_retries_without_tools_on_status_410(nim_provider):
    req = MockRequest(
        tools=[
            MockTool(
                "Task",
        ...
        "tool_calls": [
            {
                "id": "call_1",
                "type": "function",
                "function": {"name": "Task", "arguments": '{"description":"x"}'},
            }
        ],
    }
    tool_result = {"role": "tool", "tool_call_id": "call_1", "content": "ok"}
    req.messages = [MockMessage("user", "Hi")]
    body = nim_provider._build_request_body(req)
    body["messages"].extend([tool_call_only, tool_result])
    ...
    mock_chunk.usage = MagicMock(completion_tokens=3)

    async def mock_stream():
        yield mock_chunk

    error = _make_status_error(410, "Function 'abc': Not found for account")
    ...
    with patch.object(
        ...
        mock_create.side_effect = [error, mock_stream()]
        with patch.object(
            nim_provider, "_build_request_body", return_value=body
        ) as mock_build:
            events = [e async for e in nim_provider.stream_response(req)]
    ...
    assert "tools" not in second_call
    assert all(msg.get("role") != "tool" for msg in second_call["messages"])
    assert all("tool_calls" not in msg for msg in second_call["messages"])
    assert any("Recovered" in event for event in events)


@pytest.mark.asyncio
async def test_stream_response_retries_without_tools_on_function_not_found_400(
    nim_provider,
):
    req = MockRequest(
        tools=[
    ...
    error = _make_bad_request_error("Function 'abc': Not found for account")
