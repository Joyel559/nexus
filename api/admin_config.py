...
MASKED_SECRET = "********"
MANAGED_ENV_RELATIVE = Path(".config") / "nexus" / ".env"
...
SECTIONS: tuple[ConfigSectionSpec, ...] = (
    ...
    ConfigSectionSpec(
        "agents",
        "Agents",
        "Claude skills/agents discovery and sync settings.",
    ),
    ConfigSectionSpec(
        "diagnostics",
        "Diagnostics",
...
FIELDS: tuple[ConfigFieldSpec, ...] = (
    ...
    ConfigFieldSpec(
        ...
        "providers",
        "secret",
        settings_attr="github_models_api_key",
        secret=True,
    ),
    ConfigFieldSpec(
        "ANTIGRAVITY_API_KEY",
        ...
        "providers",
        "secret",
        settings_attr="antigravity_api_key",
        secret=True,
        description="Token used by opencode-antigravity-auth backend.",
    ),
    ConfigFieldSpec(
        "ANTIGRAVITY_BASE_URL",
        "Antigravity Base URL",
        "providers",
        settings_attr="antigravity_base_url",
    ...
    ConfigFieldSpec(
        ...
        "providers",
        settings_attr="google_oauth_client_id",
        default="",
    ),
    ConfigFieldSpec(
        ...
        "providers",
        "secret",
        settings_attr="google_oauth_client_secret",
        default="",
        secret=True,
    ),
    ConfigFieldSpec(
        ...
        "Google OAuth Scopes",
        "providers",
        settings_attr="google_oauth_scopes",
    ...
    ConfigFieldSpec(
        ...
        "providers",
        settings_attr="github_oauth_client_id",
        default="",
    ),
    ConfigFieldSpec(
        ...
        "providers",
        "secret",
        settings_attr="github_oauth_client_secret",
        default="",
        secret=True,
    ),
    ConfigFieldSpec(
    ...
    ConfigFieldSpec(
        "GATEWAY_STATE_DB_PATH",
        "Gateway SQLite Path",
        "runtime",
        settings_attr="gateway_state_db_path",
        default=".config/nexus/gateway.db",
    ),
    ConfigFieldSpec(
        "GATEWAY_STORAGE_BACKEND",
        "Gateway Storage Backend",
        "runtime",
        "select",
        settings_attr="gateway_storage_backend",
        default="sqlite",
        options=("sqlite", "postgres"),
    ),
    ConfigFieldSpec(
        "GATEWAY_POSTGRES_DSN",
        "Gateway Postgres DSN",
        "runtime",
        "secret",
        settings_attr="gateway_postgres_dsn",
        default="",
        secret=True,
        advanced=True,
        description="Reserved for future PostgreSQL storage backend support.",
    ),
    ConfigFieldSpec(
        "GATEWAY_RETENTION_DAYS",
        "Gateway Retention Days",
        "runtime",
        "number",
        settings_attr="gateway_retention_days",
        default="14",
    ),
    ConfigFieldSpec(
        "GATEWAY_QUEUE_MAX_INFLIGHT",
        "Gateway Queue Max Inflight",
        "runtime",
        "number",
        settings_attr="gateway_queue_max_inflight",
        default="128",
    ),
    ConfigFieldSpec(
        "GATEWAY_QUEUE_MAX_QUEUED",
        "Gateway Queue Max Queued",
        "runtime",
        "number",
        settings_attr="gateway_queue_max_queued",
        default="512",
    ),
    ConfigFieldSpec(
        "GATEWAY_QUEUE_ACQUIRE_TIMEOUT_MS",
        "Gateway Queue Acquire Timeout (ms)",
        "runtime",
        "number",
        settings_attr="gateway_queue_acquire_timeout_ms",
        default="30000",
    ),
    ConfigFieldSpec(
        "GATEWAY_QUEUE_BACKEND",
        "Gateway Queue Backend",
        "runtime",
        "select",
        settings_attr="gateway_queue_backend",
        default="local",
        options=("local", "redis"),
    ),
    ConfigFieldSpec(
        ...
        "runtime",
        "secret",
        settings_attr="gateway_redis_url",
        default="",
        secret=True,
        advanced=True,
        description="Required only when Gateway Queue Backend is set to redis.",
    ),
    ConfigFieldSpec(
        "GATEWAY_REDIS_QUEUE_PREFIX",
        "Gateway Redis Queue Prefix",
        "runtime",
        settings_attr="gateway_redis_queue_prefix",
        default="fcc-gateway",
        advanced=True,
    ),
    ConfigFieldSpec(
        "GATEWAY_EVENT_BUS_BACKEND",
        "Gateway Event Bus Backend",
        "runtime",
        "select",
        settings_attr="gateway_event_bus_backend",
        default="local",
        options=("local", "redis"),
    ),
    ConfigFieldSpec(
        "GATEWAY_REDIS_EVENT_CHANNEL",
        "Gateway Redis Event Channel",
        "runtime",
        settings_attr="gateway_redis_event_channel",
    ...
    ConfigFieldSpec(
        ...
        "runtime",
        settings_attr="gateway_public_base_url",
        default="",
        advanced=True,
        description=(
            "Optional absolute URL used for OAuth callbacks in Docker/VPS deployments. "
    ...
    ConfigFieldSpec(
        "AGENTS_REPO_PATHS",
        "Agents Repository Paths",
        "agents",
        settings_attr="agents_repo_paths",
        description="Comma-separated local repository paths to scan for agents.",
    ),
    ConfigFieldSpec(
        "AGENTS_SYNC_TARGETS",
        "Agents Sync Targets",
        "agents",
        settings_attr="agents_sync_targets",
        description="Comma-separated output directories synced for Claude Code agents.",
    ),
    ConfigFieldSpec(
        "AGENTS_CUSTOM_ROOT",
        "Custom Agents Root",
        "agents",
        settings_attr="agents_custom_root",
        description="Filesystem root used for imported custom agents.",
    ),
    ConfigFieldSpec(
        "AGENTS_AUTODETECT_ON_STARTUP",
        "Autodetect On Startup",
        "agents",
        "boolean",
        settings_attr="agents_autodetect_on_startup",
        default="true",
    ),
    ConfigFieldSpec(
        "AGENTS_AUTO_SYNC_ON_STARTUP",
        "Auto Sync On Startup",
        "agents",
        "boolean",
        settings_attr="agents_auto_sync_on_startup",
        default="false",
    ),
    ConfigFieldSpec(
