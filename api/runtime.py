...
from api.gateway import GatewayRuntime
...
class AppRuntime:
    ...
    _gateway_runtime: GatewayRuntime | None = field(default=None, init=False)
    ...
    async def startup(self) -> None:
        ...
        self._gateway_runtime = GatewayRuntime.from_settings(self.settings)
        self.app.state.provider_registry = self._provider_registry
        self.app.state.gateway_runtime = self._gateway_runtime
        try:
            warn_if_process_auth_token(self.settings)
            self._gateway_runtime.sweep_expired_cooldowns()
            self._gateway_runtime.start_background_workers(self._provider_registry)
    ...
    async def shutdown(self) -> None:
        ...
        if self._gateway_runtime is not None:
            try:
                await self._gateway_runtime.stop_background_workers()
                self._gateway_runtime.close()
            except Exception as e:
                if verbose:
                    logger.warning("Gateway runtime close failed: {}", e)
                else:
                    logger.warning(
                        "Gateway runtime close failed: exc_type={}",
                        type(e).__name__,
                    )
